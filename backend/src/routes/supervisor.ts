...
router.post('/inspections/:id/review', async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const reports = inspection.reports as any[] | undefined
    const finalContent = editedReport || (inspection as any).reportVersions?.[0]?.content || (reports?.length ? (reports[0] as any)?.summary : null) || inspection.aiAnalysis
...
router.get('/inspections/:id', async (req: AuthRequest, res) => {
  ...
  const ordi = await calculateOrdi(inspection)
  const reports = inspection.reports as any[] | undefined
  const finalReport = (inspection as any).reportVersions?.find(
    (v: any) => v.versionType === 'FINAL_APPROVED' || v.versionType === 'FINAL'
  )?.content || null
  res.json({ inspection, ordi, aiDraft: inspection.aiAnalysis, finalReport })
})
...
router.get('/trust', async (req: AuthRequest, res) => {
  ...
  res.json({ inspectors: inspectors.map((u: any) => ({ id: u.id, name: u.name, currentTrust: u.trustScore?.score ?? null, calculationSource: 'Trust Evolution Agent (persisted)', history: (u as any).trustHistory, inspections: (u as any).assignedInspections })) })
})
...
router.get('/executive', async (req: AuthRequest, res) => {
  ...
  res.json({ kpis: { totalInspections: inspections.length, completedInspections: completed.length, pendingInspections: inspections.length - completed.length, criticalViolations: inspections.reduce((total, inspection) => total + inspection.violations.filter((violation: any) => violation.severity === 'CRITICAL').length, 0), highRiskSites: new Set(inspections.filter(inspection => (latest.get(inspection.id)?.score || 0) >= 50).map(inspection => inspection.siteId)).size, averageComplianceRate: average(checklistItems.map(item => item.status === 'COMPLIANT' ? 100 : 0)), inspectorPerformance: average(trustScores) }, departmentPerformance, trends, alerts, insights, recentActions: reviewActions.map((action: any) => ({ id: action.id, action: action.action, site: action.inspection.site.name, reviewer: action.reviewer.name, status: action.newStatus, createdAt: action.createdAt })) })
})
...
router.get('/inspections/:id/export/pdf', async (req: AuthRequest, res) => {
  ...
  doc.fontSize(15).text('Final Report').fontSize(12).text(reportVersions?.[0]?.content || inspection.aiAnalysis || 'No report content available').moveDown()
  doc.fontSize(15).text('Checklist').fontSize(12); inspection.checklists.forEach((item: any) => doc.text(`${item.itemLabel}: ${item.status}`)); doc.moveDown(); doc.fontSize(15).text('Violations').fontSize(12); inspection.violations.forEach((item: any) => doc.text(`${item.severity}: ${item.description}`)); doc.end()
})
...
router.get('/memory/graph', async (req: AuthRequest, res) => {
  ...
  inspections.forEach(i => { add(`inspection:${i.id}`, 'Inspection', i.id); add(`site:${i.siteId}`, 'Site', i.site.name); add(`inspector:${i.inspectorId}`, 'Inspector', i.inspector.name); edges.push({ from: `inspector:${i.inspectorId}`, to: `inspection:${i.id}`, type: 'inspected' }, { from: `inspection:${i.id}`, to: `site:${i.siteId}`, type: 'at_site' }); i.images.forEach((x: any) => { add(`evidence:${x.id}`, 'Evidence', x.description || x.id); edges.push({ from: `inspection:${i.id}`, to: `evidence:${x.id}`, type: 'has_evidence' }) }); i.violations.forEach((x: any) => { add(`violation:${x.id}`, 'Violation', x.description); edges.push({ from: `inspection:${i.id}`, to: `violation:${x.id}`, type: 'flagged' }) }); i.reviews.forEach((x: any) => { add(`review:${x.id}`, 'Review', x.approved === true ? 'Approved' : x.approved === false ? 'Rejected' : 'Pending'); edges.push({ from: `review:${x.id}`, to: `inspection:${i.id}`, type: 'reviewed' }) }) })
...
router.get('/timeline/:id', async (req: AuthRequest, res) => {
  ...
  const events = [{ type: 'Inspection assigned', timestamp: inspection.createdAt }, ...(inspection as any).checklists.map((i: any) => ({ type: `Checklist updated: ${i.itemLabel}`, timestamp: i.updatedAt })), ...(inspection as any).images.map((i: any) => ({ type: 'Evidence uploaded', timestamp: i.uploadedAt })), ...(inspection as any).verificationFindings.map((i: any) => ({ type: 'AI verification', timestamp: i.createdAt })), ...(inspection as any).complianceMemoryEvents.map((i: any) => ({ type: i.eventType.replace(/_/g, ' '), timestamp: i.occurredAt })), ...(inspection as any).reviewActions.map((i: any) => ({ type: `Supervisor: ${i.action.replace(/_/g, ' ')}`, timestamp: i.createdAt })), ...notifications.map(i => ({ type: `Notification: ${i.title}`, timestamp: i.createdAt })), ...auditLogs.map(i => ({ type: `Audit: ${i.action}`, timestamp: i.createdAt }))].sort((a: any, b: any) => +new Date(a.timestamp) - +new Date(b.timestamp))
  res.json({ events })
})
